...
async def sync_guest_data(
    ...
    if state.won:
        streak = (await USStatedleStateRepository(session).get_current_streak(user.id)) + 1
        elapsed = None
...
async def _do_ask_question(
    ...
    if user is None:
        ...
        if question_create.valid:
            await record_guest_action(session, request, response, "us_statedle", day_state.id, question=True)
