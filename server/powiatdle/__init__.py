...
async def sync_guest_data(
    ...
    state = await lock_question_state(session, PowiatdleState, user.id, day_powiat.id) or state
    ...
    if sync_state is not None and not is_valid_synced_game_state(
        ...
        won=sync_state.won,
        correct_guesses=[
            guess.powiat_id == day_powiat.powiat_id for guess in sync_data.guesses
        ],
