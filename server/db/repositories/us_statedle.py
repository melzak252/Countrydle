...
class USStatedleDayRepository:
    ...
    async def get_day_us_state_by_date(self, day_date) -> Optional[USStatedleDay]:
        result = await self.session.execute(
            select(USStatedleDay).where(USStatedleDay.date == day_date)
        )
        return result.scalar_one_or_none()
            ...
            stmt = (
                ...
                .where(
                    and_(
                        ...
                        or_(USStatedleState.questions_asked > 0, USStatedleState.guesses_made > 0),
                    )
                )
