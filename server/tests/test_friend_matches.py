...
def test_first_timeout_passes_second_consecutive_timeout_forfeits_and_outage_interrupts():
    game, seats = active_game()
    timeout_moves = []
    assert expire_match(game, seats, NOW + timedelta(seconds=121), timeout_moves=timeout_moves)
    ...
    transition(game, seats, seats[1], action("pass"), NOW + timedelta(seconds=122))
    assert expire_match(game, seats, NOW + timedelta(seconds=244))
    assert game.result == "forfeit" and game.winner_id == seats[1].id
    game, seats = active_game()
    assert expire_match(game, seats, NOW + timedelta(seconds=150))
