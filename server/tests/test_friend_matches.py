...
from friend_matches.service import transition, project_move, project_guidance, expire_match
...
def active_game():
    game = FriendMatch(id=str(uuid4()), mode="countrydle", status="active", phase="thinking", version=1,
                       turn=1, move_ordinal=0, deadline=NOW + timedelta(seconds=45))
...
def test_first_timeout_passes_second_consecutive_timeout_forfeits_and_outage_interrupts():
    game, seats = active_game()
    timeout_moves = []
    assert expire_match(game, seats, NOW + timedelta(seconds=46), timeout_moves=timeout_moves)
    ...
    transition(game, seats, seats[1], action("pass"), NOW + timedelta(seconds=47))
    assert expire_match(game, seats, NOW + timedelta(seconds=93))
    assert game.result == "forfeit" and game.winner_id == seats[1].id
    game, seats = active_game()
    assert expire_match(game, seats, NOW + timedelta(seconds=90))
    assert game.result == "interrupted" and game.winner_id is None


def test_owner_timeout_is_not_an_invented_human_unknown_answer():
    game, seats = active_game()
    question = transition(game, seats, seats[0], action("ask", question="Is it in Europe?"), NOW)
    assert expire_match(game, seats, NOW + timedelta(seconds=31), pending_move=question)
    assert question.answer is None and question.revisions == [] and question.timed_out is True
    assert game.active_player_id == seats[1].id and seats[1].timeout_count == 1
    transition(game, seats, seats[1], action("pass"), NOW + timedelta(seconds=32))
...
def test_reply_deadline_confirms_pending_winner_as_solved():
    game, seats = active_game()
    transition(game, seats, seats[0], action("guess", entity_id="POL"), NOW, entity=POLAND)
    assert expire_match(game, seats, NOW + timedelta(seconds=46))
