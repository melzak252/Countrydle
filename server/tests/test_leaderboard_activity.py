...
@pytest.fixture(params=CASES, ids=['country', 'us_states', 'powiaty', 'wojewodztwa', 'continental'])
...
def add_day(session, day_cls, day_id, puzzle_date):
    values = {'id': day_id, 'date': puzzle_date}
    if day_cls is ContinentalDay:
        values.update(continent=ContinentCode.EUROPE, country_id=1)
...
async def test_monthly_rankings_exclude_idle_accounts_but_keep_zero_point_players(leaderboard_db):
    ...
    add_day(session, day_cls, 2, today.replace(day=1) - timedelta(days=1))
    session.add_all([
    ...
    assert {entry.id for entry in result} == {2, 3, 5}
    assert result[0].id == 5
    assert result[0].points == 2000
...
async def test_average_rankings_do_not_use_idle_rows_to_meet_minimum_games(leaderboard_db):
    ...
    for i in range(minimum):
        add_day(session, day_cls, i + 1, date.today() - timedelta(days=i))
        session.add_all([
