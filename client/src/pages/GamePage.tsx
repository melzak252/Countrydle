...
// removed unused GuestProgress and useDailyDate
      ...
      <div className="pointer-events-none absolute left-4 bottom-4 z-[1000] w-80 sm:w-92 max-w-[calc(100vw-2rem)]">
        {!isChatOpen ? (
          ...
          <div className="pointer-events-auto flex h-[48vh] sm:h-[52vh] max-h-[48vh] sm:max-h-[52vh] flex-col overflow-hidden rounded-sm border border-white/15 bg-obsidian-900/85 shadow-2xl backdrop-blur-md transition-all">
            ...
            {activeChatTab === 'questions' && (
              <div ref={questionsContainerRef} className="flex-1 min-h-0 overflow-y-auto p-3 space-y-2.5 text-xs custom-scrollbar overscroll-contain">
                {sortedQuestions.length === 0 ? (
                  <div className="py-7 text-center text-zinc-400 space-y-2 border border-dashed border-white/10 rounded-sm p-4">
                    ...
                    <p className="text-xs leading-relaxed text-zinc-500 max-w-[240px] mx-auto">
                      Submit a yes-or-no inquiry below (e.g. &quot;Is it in Europe?&quot; or &quot;Does it have coastal access?&quot;).
                    </p>
                  </div>
                ) : (
                  sortedQuestions.map((q, index) => {
                    ...
                    return (
                      <div key={q.id} className="space-y-1.5 border-b border-white/5 pb-2.5 last:border-0 last:pb-0">
                        {/* 1. Player Inquiry Message (Right-aligned speech bubble) */}
                        ...
                        {/* 2. Atlas Dispatch Response (Left-aligned speech bubble, NO nested boxes) */}
